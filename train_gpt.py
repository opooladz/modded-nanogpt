...
class Block(nn.Module):
    def __init__(self, dim: int, layer_idx: int, ssm_state_size: int):  # Added ssm_state_size
        super().__init__()
        # Replace attention with SSM (skip for layer 7)
        self.attn = CustomComplexSSM(dim, ssm_state_size, dim) if layer_idx != 7 else None
        self.mlp = MLP(dim)
        # Keep skip connection weights
        self.lambdas = nn.Parameter(torch.tensor([1., 0.]))  # For x + x0 mixing

    def forward(self, x: Tensor, x0: Tensor):  # Removed ve and block_mask
        # Mix current state with initial embedding
        x = self.lambdas[0] * x + self.lambdas[1] * x0
        
        # SSM branch
        if self.attn is not None:
            ssm_out, _ = self.attn(norm(x))  # Apply RMSNorm before SSM
            x = x + ssm_out  # Residual connection
...
class GPT(nn.Module):
    def __init__(self, vocab_size: int, num_layers: int, model_dim: int, 
                 max_seq_len: int, ssm_state_size: int = 64):  # Added SSM param
        super().__init__()
        # Embedding layer only (removed value_embeds)
        self.embed = nn.Embedding(vocab_size, model_dim)
        
        # SSM-based blocks
        self.blocks = nn.ModuleList([
            Block(model_dim, i, ssm_state_size)  # Pass SSM size
            for i in range(num_layers)
        ])
        
        # Output head (keep FP8 optimizations)
        ...
        # Skip connections (keep original U-net logic)
    ...
    def forward(self, input_seq: Tensor, target_seq: Tensor):
        ...
        for i, block in enumerate(self.blocks):
            ...
            x = block(x, x0)  # No ve/block_mask needed
            if i < n:
        ...
        logits = 30 * torch.sigmoid(logits / (7.5 * x.size(-1)**0.5))  # Softcap
